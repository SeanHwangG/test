...
int run_tests(vector<Point> &kdt_build_points, vector<Point> &test_cases, vector<Point> &solutions) {
  ...
  unsigned int targetHeight = kdt_build_points.size() == 0 ? 0 : (unsigned int)log2(kdt_build_points.size()) + 1;

  if (tree->height() != targetHeight) {
    cerr << "Test failed for tree height" << endl;
    cerr << "\tExpected: " << targetHeight << "\tActual: " << tree->height() << endl;
    error_count++;
  }

  vector<Point>::iterator test_it = test_cases.begin();
  vector<Point>::iterator solution_it = solutions.begin();
  for (; test_it != test_cases.end() && solution_it != solutions.end(); test_it++, solution_it++) {
    Point find_result = *tree->findNearestNeighbor(*test_it);

    if (find_result != *solution_it) {
      cerr << "Test failed for find of point " << *test_it << "\n\tExpected: " << *solution_it << "\n\tActual: " << find_result
           << endl;
      error_count++;
    }
  }
...
#define ASSERT_NOTHING_FOUND(T, P)                                   \
  if (T.findNearestNeighbor(P) != T.end()) {                         \
