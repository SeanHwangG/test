...
int main() {
  ...
  if (distance != 1.0) {
    cout << "Incorrect value for Point.distance" << endl << "\texpected: " << 1.0 << endl << "\tactual: " << distance << endl;
    return -1;
  }

  distance = Point::squareDistance(Point(-1.0, 4.0), Point(1.0, 4.0));
  if (distance != 4.0) {
    cout << "Incorrect value for Point.distance" << endl << "\texpected: " << 4.0 << endl << "\tactual: " << distance << endl;
    return -1;
  }

  distance = Point::squareDistance(Point(-1.0, 2.0), Point(0.0, 1.0));
  if (distance != 2.0) {
    cout << "Incorrect value for Point.distance" << endl << "\texpected: " << 2.0 << endl << "\tactual: " << distance << endl;
    return -1;
  }
  ...
  for (; nvit != nearby_points.end() && vit != ven; nvit++, vit++) {
    auto found = kdtree.findNearestNeighbor(*nvit);
    if (*found != *vit) {
      cout << "looking for " << *vit << " but found " << *found;
      return -1;
    }
  }
