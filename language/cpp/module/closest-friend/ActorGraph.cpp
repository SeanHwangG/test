...
ActorGraph::~ActorGraph(void) {
  for (auto it : actors) delete it.second;
  for (auto it : movies) delete it.second;
}
...
bool ActorGraph::addMovie(string actor, string movie, int year) {
  if (!actors.count(actor)) actors[actor] = new Actor(actor);
  if (!movies.count(movie)) movies[movie] = new Movie(movie, year);
...
string ActorGraph::findPath(Actor* a, Actor* b) {
  if (a == nullptr || b == nullptr) return "Not Found";
