...
PointCloud::PointCloud(std::string objFilename, GLfloat pointSize) : pointSize(pointSize)
{
  ...
  points = rescale(points, 14);
  normals = rescale(normals, 1);
...
void PointCloud::spin(float deg, glm::vec3 rotM)
{
  // Update the model matrix by multiplying a rotation matrix
  model = glm::rotate(model, glm::radians(deg), rotM);
}

vector<glm::vec3> PointCloud::rescale(vector<glm::vec3> points, float constant)
{
