...
def get_source(problemList,option,userId,cookies):
  ...
  for problem in problemList:
    response=requests.get(f"https://www.acmicpc.net/status?from_mine=1&problem_id={problem}&user_id={userId}", cookies=cookies).text
    soup = BeautifulSoup(response,'html.parser')
    ...
    response=requests.get(f"https://www.acmicpc.net/submit/{problem}/{solutionId}", cookies=cookies).text
    soup=BeautifulSoup(response,'html.parser')
    ...
    if not os.path.exists('./{}/{}~{}'.format(dirname,int(problem)//1000*1000,(int(problem)//1000+1)*1000-1)):
      os.mkdir(f'./{dirname}/{int(problem)//1000*1000}~{(int(problem)//1000+1)*1000-1}')
    file=open(f'./{dirname}/{int(problem)//1000*1000}~{(int(problem)//1000+1)*1000-1}/{problem}.{extension}','w')
    file.write(source)
    file.close()
