...
* CMakeList.txt: input to the CMake build system for building software packages
...
* django.django-admin: aka python3 manage.py
  * Only allowing admin docs access via HTTPS, limiting admin docs access based on IP
  ...
  * sqlflush: returns SQL statements to return all tables in database to the state they were in just after they were installed
...
* make
  ...
  * -s: silent error
  * -j `n`: Set to `n` number of core ([ex] $(proc))
...
* Model
  ...
  * Use Meta.fields instead of Meta.exclude -> know exactly what fields we are exposing
  ...
  | ------------- | ------------------------------------------------------------ | ----------------------------------- |
  | No inherit    | Easiest to understand                                        | duplicated fields across models     |
...
* sys.path: a list of strings that determines the interpreter’s search path for modules
  ...
  ```py
  sys.path.insert(0, "/home/myname/pythonfiles") # Append to PYTHONPATH
  ```
...
* npm: install everything in package.json
  ...
  * install
  ...
  npx create-react-app my-app  # don’t have to download create-react-app
  cd my-app
  npm start
  ```
...
* pad: elements input and output, where you can connect other elements
