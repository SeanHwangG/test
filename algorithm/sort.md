...
* sort CLI: sort or merge records (lines) of text and binary files
...
* sorted(`iterable`) -> list: Sort iterable and create new iterable
  * key=None: custom key function can be supplied to customize the sort order
...
def RMerge(a1, …, ak, b1, …, bl: sorted lists)
...
{% include '.sort-key.prob' %}

### Stable sort

{% include '.stable-sort.prob' %}

### Combine

* Tuple can be compared in cpp / python

{% include '.sort-combine.prob' %}
