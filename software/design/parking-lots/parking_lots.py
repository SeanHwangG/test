...
class ParkingLot:
  ...
  class __OnlyOne:
    def __init__(self, name, address):
      ...
      #    this map should tell how many parking spots are there on each floor. should also initialize max spot counts
      ...
      self.__active_tickets = {} # all active parking tickets, identified by their ticket_number
  ...
  def get_new_parking_ticket(self, vehicle):
    ...
    # synchronizing to allow multiple entrances panels to issue a new parking ticket without interfering with each other
    self.__lock.acquire()
  ...
  def is_full(self, type):
    ...
    return (self.__compact_spot_count + self.__large_spot_count + self.__electric_spot_count) >=
           (self.__max_compact_count + self.__max_large_count + self.__max_electric_count)
