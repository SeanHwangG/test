...
* Hadlock's algorithm
  * First try paths with no detour (only direction towards goal), then if necessary try paths with 1, 2 detour step...
  * Distance then is the Manhattan distance plus twice the number of detour steps
...
* Kruskal Algorithm: Pick the next lightest edge that doesn't create a cycle
